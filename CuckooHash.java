...
public class CuckooHash<K, V> {
	...
	//this doesn't work
 	public void put(K key, V value) {
		...
		while (n < CAPACITY && !success) {
			if (hashMode == 1) {
				if (table[hash1(currentKey)] == null) {
					table[hash1(currentKey)] = new Bucket<>(currentKey, currentValue);
					success = true;
				} else {
					if (!(hash1(currentKey) == hash1(table[hash1(currentKey)].getBucKey()))) {
						hashMode = 2;
					}
					tempKey = table[hash1(currentKey)].getBucKey();
					tempValue = table[hash1(currentKey)].getValue();
					table[hash1(currentKey)].bucKey = currentKey;
					table[hash1(currentKey)].value = currentValue;

				}
			} else {
				if (table[hash2(currentKey)] == null) {
					table[hash2(currentKey)] = new Bucket<>(currentKey, currentValue);
					success = true;
				} else {
					if (!(hash2(currentKey) == hash2(table[hash2(currentKey)].getBucKey()))) {
						hashMode = 1;
					}
					tempKey = table[hash2(currentKey)].getBucKey();
					tempValue = table[hash2(currentKey)].getValue();
					table[hash2(currentKey)].bucKey = currentKey;
					table[hash2(currentKey)].value = currentValue;
