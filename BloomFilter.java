...
class BloomFilter {
    ...
    public boolean contains(String s) {
        ...
        //while loop emulates the for loop in the add function, but with a break condition
        int n = 0;
        while (n < noHashes && contained) {
            //check for each hash being present in the BitMap, terminate if a missing one is detected
            contained = data.get((int) hashCode(s, n) & this.hashMask);
            n++;
        }
